...
fn main() -> anyhow::Result<()> {
    let window = winman::Window::new("16.5 - Light Casters: Spot Soft", 1920, 1080)?;
    ...
    let object_light_cutoff_location = object_shader_program.locate_uniform("light.cutoff")?;
    let object_light_outer_cutoff_location =
        object_shader_program.locate_uniform("light.outer_cutoff")?;
    ...
    let object_light_attenuation_linear_location =
        object_shader_program.locate_uniform("light.attenuation_linear")?;
    let object_light_attenuation_quadratic_location =
        object_shader_program.locate_uniform("light.attenuation_quadratic")?;
    ...
    opengl_sys::set_uniform_f32(object_light_cutoff_location, 12.5f32.to_radians().cos())?;
    opengl_sys::set_uniform_f32(
        object_light_outer_cutoff_location,
        17.5f32.to_radians().cos(),
    )?;
    opengl_sys::set_uniform_f32(object_light_attenuation_linear_location, 0.09)?;
    opengl_sys::set_uniform_f32(object_light_attenuation_quadratic_location, 0.032)?;
    ...
    window.run(|window_size, (_, seconds_since_last_frame), events| {
        ...
        opengl_sys::set_uniform_vec3(object_light_position_location, &camera.get_position())
            .unwrap();
        opengl_sys::set_uniform_vec3(object_light_direction_location, &camera.get_direction())
            .unwrap();
